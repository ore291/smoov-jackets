...
function Department({ base_url , dept = {name: ""}, slug}) {
  const jackets = [
    {
      "#1879ae": [
        "/0/#1879ae-front.svg",
      ...
      <div className="w-full py-2 ">
        <div className="w-full flex flex-col justify-center md:items-center space-y-4 p-2 my-4">
          ...
          <h2 className="text-center md:max-w-[600px] text-[#667085] text-sm font-normal">
            Welcome to the {dept.name} final year varsity jacket sign
...
export const getServerSideProps = async ({ req, res , params}) => {
  ...
  const { data: dept } = await axios.get(origin + "/api/depts/" + params.dept);
